...
| Model ports | extension-less blob (`unstructured_data`), ZIP by magic bytes; since 2026-09-17 `model_in` can ALSO arrive as a raw path-text pointer into a dead container (platform re-serialization / registry wiring) | `_resolve_embedder` sniffs content, never suffixes; `_bundle_root` accepts zip / dir / JSON payload / raw path text, with actionable errors for dead pointers; inference `model_out` re-emits a transportable payload so chaining cannot produce dead pointers |
